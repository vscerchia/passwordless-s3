...
class S3Store extends TokenStore {
    ...
    */
            ...
            if (err) {
                ...
                if (Date.now() > ttl) {
                    this.invalidateUser(uid, () => {
                        // I think failing is OK here. It means we were unable to clean up stale data in S3.
                    });
                    callback(null, false, null);
                }
                this._verify(token, hashedToken, (err, same) => {
                    if (err) {
                        callback(err, false, null);
                    }
                    else if (!same) {
    ...
    storeOrUpdate(token, uid, msToLive, originUrl, callback) {
        ...
        this._hash(token, (err, hashedToken) => {
            if (err) {
                callback(err);
            }
            else {
                ...
                this._s3Client.putObject(params, (err) => {
                    if (err) {
                        callback(err);
                    }
                    else {
    ...
    invalidateUser(uid, callback) {
        ...
        this._s3Client.deleteObject(params, (err) => {
            if (err) {
                callback(err);
            }
            else {
    ...
    _hash(token, callback) {
        bcrypt.hash(token, 10, callback);
    }

    _verify(token, hashedToken, callback) {
